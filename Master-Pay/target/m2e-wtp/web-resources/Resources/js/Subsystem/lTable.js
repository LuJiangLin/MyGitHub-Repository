        ...
        var idsStr="";
  		...
  		self.getCheckboxIds=function (){
   		...
   		self.init = function () {
            ...
            for(var m in data){
            	thb+="<tr onmouseover='style.backgroundColor=\"#F2F2F2\"' onmouseout='style.backgroundColor=\"#FFFFFF\"'>";
            	...
            	for(var k=0;k<thids.length;k++){
            		...
            		for(var n in data[m]){
	        			if(n==thids[k]){
	        				...
	        				if(thids[k] == time[inm]){   //时间转化
	        					inm++;
	        					//时间转化方法timestampToTime
	        					thb+=('<td>'+(timestampToTime(data[m][n])==null?"":timestampToTime(data[m][n]))+'</td>');
	        				}else{
			...
			htmls="<table  class='table table-border table-bordered table-bg table-sort tabletop-col'>"+(ths+thb)+"</table>";
			self.$container.html(htmls);
	   		
			$("th,td").css({"height":"30px","line-height":"30px","margin":"5px,0"});
	...
	$.lPaging = function (el,options) {
		...
		self.init = function () {
			...
			if(pageNumber!=totalPage){
				ul+="<li><a href='javascript:;' pv='"+(Number(pageNumber) + Number(1))+"'>下一页</a></li>";
